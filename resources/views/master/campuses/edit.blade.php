    ...
    <div class="py-8 bg-slate-50/50 min-h-screen">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center gap-4 mb-2">
                ...
                <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Campus: <span class="text-[#009B77]">{{ $campus->name }}</span></h2>
            </div>

            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="h-2 w-full bg-gradient-to-r from-[#009B77] to-emerald-500"></div>
                
                <form action="{{ route('master.campuses.update', $campus->id) }}" method="POST" class="p-6 sm:p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" id="location" name="location" value="{{ old('location', $campus->location) }}"
                                    class="w-full rounded-xl border-slate-200 focus:border-[#009B77] focus:ring focus:ring-[#009B77]/20 transition-all shadow-sm bg-slate-50 focus:bg-white" 
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="3"
                                    ...
                                    placeholder="Optional details about this campus...">{{ old('description', $campus->description) }}</textarea>
                            ...
                            <div class="md:col-span-2">
                                <label class="flex items-center gap-3 p-4 rounded-xl border border-slate-200 bg-slate-50/50 cursor-pointer hover:bg-slate-50 transition-colors">
                                    <input type="checkbox" name="is_active" value="1" class="w-5 h-5 rounded border-slate-300 text-[#009B77] focus:ring-[#009B77] transition-all" {{ old('is_active', $campus->is_active) ? 'checked' : '' }}>
                                    <div>
